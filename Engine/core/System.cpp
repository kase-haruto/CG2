...
void System::NonDepthObject3D(){// InputLayoutの設定
	D3D12_INPUT_ELEMENT_DESC inputElementDescs[3] = {};
	inputElementDescs[0].SemanticName = "POSITION";
	inputElementDescs[0].SemanticIndex = 0;
	inputElementDescs[0].Format = DXGI_FORMAT_R32G32B32A32_FLOAT;
	...
	// RasterizerStateの設定
	D3D12_RASTERIZER_DESC rasterizeDesc {};
	rasterizeDesc.CullMode = D3D12_CULL_MODE_FRONT;  // 裏面カリング
	rasterizeDesc.FillMode = D3D12_FILL_MODE_SOLID; // ソリッドフィル
	rasterizeDesc.FrontCounterClockwise = FALSE;    // 時計回りが前面
	rasterizeDesc.DepthClipEnable = TRUE;           // 深度クリップを有効
	...
	depthStencilDesc.DepthEnable = true;
	depthStencilDesc.DepthWriteMask = D3D12_DEPTH_WRITE_MASK_ALL;
	depthStencilDesc.DepthFunc = D3D12_COMPARISON_FUNC_ALWAYS;   // 常に描画するように比較関数を設定
	...
	// パイプラインステートオブジェクトの作成
	if (!pipelineStateManager_->CreatePipelineState(NoneDepth, L"Object3d.VS.hlsl", L"Object3d.PS.hlsl", rootSignatureDesc, psoDesc)){
		// エラーハンドリング
		return;
	}
}
