...
void System::NonTexturesObjectPipelines(){
    // InputLayoutの設定
    D3D12_INPUT_ELEMENT_DESC inputElementDescs[3] = {};
    inputElementDescs[0].SemanticName = "POSITION";
    inputElementDescs[0].SemanticIndex = 0;
    ...
    // シェーダの読み込み
    if (!shaderManager_->LoadShader(NonTextureObject, L"NonTextureObject.VS.hlsl", L"NonTextureObject.PS.hlsl")){
        // シェーダの読み込みに失敗した場合のエラーハンドリング
        return;
    }
    ...
    psoDesc.pRootSignature = rootSignature.Get();
    psoDesc.InputLayout = inputLayoutDesc;
    psoDesc.VS = {shaderManager_->GetVertexShader(NonTextureObject)->GetBufferPointer(), shaderManager_->GetVertexShader(NonTextureObject)->GetBufferSize()};
    psoDesc.PS = {shaderManager_->GetPixelShader(NonTextureObject)->GetBufferPointer(), shaderManager_->GetPixelShader(NonTextureObject)->GetBufferSize()};
    psoDesc.BlendState = blendDesc;
    psoDesc.RasterizerState = rasterizeDesc;
    psoDesc.DepthStencilState = depthStencilDesc;
    psoDesc.NumRenderTargets = 1;
    ...
    // パイプラインステートオブジェクトの作成
    if (!pipelineStateManager_->CreatePipelineState(NonTextureObject, L"NonTextureObject.VS.hlsl", L"NonTextureObject.PS.hlsl", rootSignatureDesc, psoDesc)){
        // エラーハンドリング
        return;
    }
}
