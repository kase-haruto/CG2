...
ComPtr<ID3D12Resource> RenderTarget::CreateDepthStencilResource(ComPtr<ID3D12Device> device, int32_t width, int32_t height){
...
ComPtr<ID3D12Resource> RenderTarget::CreateRenderTextureResource(
...
void RenderTarget::CreateOffscreenRTV(ComPtr<ID3D12Device> device){
    D3D12_CPU_DESCRIPTOR_HANDLE rtvStartHandle = rtvHeap_->GetCPUDescriptorHandleForHeapStart();

    const Vector4 kRenderTargetClearValue {1.0f, 0.0f, 0.0f, 1.0f}; // 赤色でクリア
    renderTextureResource_ = CreateRenderTextureResource(device, kWindowWidth, kWindowHeight,
                                                         DXGI_FORMAT_R8G8B8A8_UNORM_SRGB, kRenderTargetClearValue);
    device->CreateRenderTargetView(renderTextureResource_.Get(), &rtvDesc_, rtvStartHandle);
}

void RenderTarget::CreateOffscreenSRV(ComPtr<ID3D12Device> device, ComPtr<ID3D12DescriptorHeap> srvHeap){
...
void RenderTarget::ClearOffscreenRenderTarget(ComPtr<ID3D12GraphicsCommandList> commandList){
    D3D12_CPU_DESCRIPTOR_HANDLE rtvHandle = rtvHeap_->GetCPUDescriptorHandleForHeapStart();
    D3D12_CPU_DESCRIPTOR_HANDLE dsvHandle = dsvHeap_->GetCPUDescriptorHandleForHeapStart();
